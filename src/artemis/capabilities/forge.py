...
AUTHOR_SYSTEM = (
    ...
    "- The verify sandbox provides ONLY these third-party libraries: requests, httpx (plus "
    "pytest to run the tests). Prefer the standard library when it is simple; importing any "
    "other third-party package will fail verification.\n"
    ...
    "- RUNTIME INPUT CONVENTION (critical): at invoke time Artemis passes ALL declared inputs "
    "as ONE JSON object string in sys.argv[1] (it is `{}` when no inputs were supplied). Your "
    "tool MUST parse it, e.g. `data = json.loads(sys.argv[1]) if len(sys.argv) > 1 else {}`, "
    "then read each field by name from `data` (e.g. `data.get(\"calendar_id\")`) and apply your "
    "own default for any missing/empty value. NEVER treat sys.argv positionally (argv[1] is the "
    "JSON blob, not the first parameter's value).\n"
