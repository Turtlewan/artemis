...
autonomy_level: L2
---
<!-- amended 2026-06-11 per m0-m1-foundation-brain.md BLOCKs B5, F1, F7 -->
<!-- amended 2026-06-11 per Decision D5 (contracts.md Seam 10) -->
...
- [ ] Task 5: Write the plist renderer — files: `/Users/artemis-build/artemis/scripts/render_plists.py` — typed Python; reads `Settings` for the given slot (`--slot`), resolves `{REPO_DIR}`, `{UV_BIN}` (`shutil.which("uv")`), `{ENV_FILE}` via `paths.env_file(s)` (B9: use the single-source-of-truth resolver from M0-a `paths.py`), `{LOGS_DIR}` (from `paths.logs_dir`), `{RUNTIME_USER}` (from env `ARTEMIS_RUNTIME_USER`), the 4 ports, and the binary-path placeholders (`{MLX_LAUNCH_CMD}`,`{NTFY_BIN}`,`{AUDIO_SIDECAR_BIN}` from env with documented defaults), then writes rendered plists to `--out-dir` (default `deploy/launchd/rendered/{slot}/`). Output filename convention (F1): each output plist is named `com.artemis.{slot}.{service}.plist` (e.g. `com.artemis.dev.brain.plist`) — spec this naming exactly so deploy.sh and RUNBOOK steps reference the correct filename. Fails loudly (`SystemExit`) on any unresolved `{...}` placeholder remaining. — done when: `uv run mypy --strict scripts/render_plists.py` passes and running it for `dev` emits 4 plists named `com.artemis.dev.{brain,mlx,ntfy,audio}.plist` with zero remaining `{` placeholders.

- [ ] Task 6: Write the local pipeline script — files: `/Users/artemis-build/artemis/scripts/pipeline.sh` — bash `set -euo pipefail`; runs in order: `uv run ruff check .`, `uv run ruff format --check .`, `uv run mypy --strict src`, `uv run pytest -q -m "not integration"` (unit), then the integration stage **with exit-5 guard** (B5: pytest exits 5 when `-m integration` matches nothing under `set -euo pipefail`; spec the stage as: `uv run pytest -q -m integration; rc=$?; [ "$rc" -eq 0 ] || [ "$rc" -eq 5 ] || exit "$rc"`), a migration-rehearsal STUB (`echo "migration rehearsal: no migrations in M0"`), then a smoke STUB (`echo`). Each stage prints a header; any non-zero stage (other than 5 on the integration step) aborts. — done when: `bash scripts/pipeline.sh` exits 0 on the M0 codebase.

- [ ] Task 7: Write the deploy/promote/rollback script — files: `/Users/artemis-build/artemis/scripts/deploy.sh`, `/Users/artemis-build/artemis/deploy/risky_paths.txt` — bash `set -euo pipefail`. `deploy.sh` flags: `--from <slot>` `--to <slot>` `[--full]`. Decision D5: each slot runs from its own **per-slot git worktree** (dev runs from `/Users/artemis-build/artemis`; UAT and PROD have sibling worktrees, e.g. `/Users/artemis-build/artemis-uat` and `/Users/artemis-build/artemis-prod`, added via `git worktree add`). **Promotion = advance the target slot's worktree to the next ref/tag** (e.g. `git -C <target-worktree> checkout <candidate-tag>`), NOT a redeploy of a single shared working tree. The worktree root for each slot is resolved from `ARTEMIS_WORKTREE_ROOT` (env, set per slot via the slot's `.env`). Behaviour: (1) determine risky = `--full` OR any changed file (F7: `git diff --name-only <prev-prod-tag>..HEAD` in the source-slot worktree; if the tag `artemis-prod-prev` does NOT exist — first-ever deploy — treat the change set as fully risky: set `risky=true`) matches a glob in `risky_paths.txt`; (2) run `pipeline.sh`; (3) if risky → also bootstrap a UAT instance (render UAT plists, `launchctl bootstrap`), print "FULL UAT: owner sign-off required" and wait for an explicit `--confirm` token file (`touch deploy/.uat-approved`) before promoting; (4) **backup-before-promote**: call the M0-e backup script for the target slot (placeholder call `scripts/backup.sh --slot <to>`); (5) record the promoted-from ref as the rollback point: `git -C <target-worktree> tag -f artemis-prod-prev`; (6) promote = `launchctl bootout` the target slot's jobs, **advance the target slot's worktree** to the new ref (`git -C <target-worktree> checkout <new-tag-or-ref>`), render the target plists from that worktree, `launchctl bootstrap` them; (7) `--rollback` flag: `git -C <target-worktree> checkout artemis-prod-prev`, re-render, re-bootstrap, restore the pre-promote backup. `risky_paths.txt` seeds globs: `src/artemis/security/*`, `src/artemis/identity/*`, `**/migrations/*`, `src/artemis/modules/finance/*`, `src/artemis/modules/health/*`, `src/artemis/modules/journal/*`, `src/artemis/memory/*`. — done when: `bash scripts/deploy.sh --from dev --to uat` (dry, on a mac without bootstrapping — guarded by an `ARTEMIS_DEPLOY_DRYRUN=1` that skips `launchctl` and `git worktree` ops) runs the pipeline + prints the planned actions and exits 0.
