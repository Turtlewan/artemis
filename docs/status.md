...
- **`CB-5b` map-node visual (deferred) — fan out display-research agents FIRST** (owner directive 2026-07-01, pre-authorized parallel research). How a built capability appears on the map is an open design decision (labeled node? cluster? click→detail of what it does + `uses`). CB-5b also adds the Tauri `app_capabilities_list` command + TS wrapper (deferred from CB-5a as dead-until-consumed). Today's live run made the gap concrete: a built capability has nowhere to land visually yet.
