...
| owner-rules capture | planning | 🟡 in-progress — elicitation | docs/owner-rules/ (9 files) + finance.md | **Scanned ~20 automation/rule-bearing specs (6 parallel agents) → 6 capture workbooks + index.** Captured: S1 proactivity (quiet hrs 23:30→07:15, gentle-nudge posture, hook schedule), S2 scheduling (tz=Asia/Singapore, 09:00–18:00 Mon–Fri), S3 email (VIPs Ashley/Debby, notify=legal+payment-warnings only, important≠notify, **Finance reconciliation recorded**), S4 memory (what-to-remember w/ Ashley anchor — **financial+health excluded**; A.U.D.N.=keep-both-when-unsure+dated; precision-first floor), S5 (idle window, auto-tagging=auto+precision-first, autonomy boundary **CONFIRMED**, **cloud line: general skills only / email stays local**). **Remaining: S6 productivity (recurrence/commitment vocab — mostly defaults), S5 curiosity token caps + egress allow-list, S1 weekly-review day + briefing/morning-plan overlap, S2 working_days + slot-pref.** Design gaps surfaced (→ planning): `working_days` field · Gmail Stage-1 Important-gate widen · bank-email→Finance reconciliation (`transaction.type`, settlement-exclusion — added to finance.md) · `needs_review` state on auto-taggers · `classify_safety` internal-reversible tier. | docs/owner-rules/*.md (9 new) · finance.md (modified) |
