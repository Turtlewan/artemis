...
- **Next Slice-3 spec** — wire the Tauri desktop UI (first `client/` touch), then event-based watchers (via `scheduler.emit`), and a real **secret store** (Telegram token is an env-var stopgap; architecture §7 keychain). The `should_fire` quota-budget gate and per-job acceptance remain seams (defaults: always-fire, no acceptance).
