...
- **`v2-16` Telegram transport — done (built by Codex).** `TelegramTransport` (Bot API: send + allowlisted long-poll receive) + `telegram_from_env`; `uv run artemis` env-selects Telegram else console. Hermetically tested (`httpx.MockTransport`). mypy 72 files · 113 tests · ruff clean. **Not yet run live** (manual go-live — see Open Questions).
- **Next:** wire the Tauri desktop UI (first touch of `client/`) → event-based watchers (via `scheduler.emit`) → a schedule-management CLI → a real secret store (Telegram token is an env stopgap).
...
- **Next Slice-3 spec** — wire the Tauri desktop UI (first `client/` touch), then event-based watchers (via `scheduler.emit`), a schedule-management CLI (`main()` only starts the loop today), and a real **secret store** (Telegram token is an env-var stopgap; architecture §7 keychain). The `should_fire` quota-budget gate and per-job acceptance remain seams (defaults: always-fire, no acceptance).
