...
|------|------|-------|------|------------|-------------|
| brain-Codex build batch | coding | 🟢 QUEUED — build via **Codex CLI** (owner: use Codex/gpt-5.5, not Claude) | docs/changes/ + docs/bring-up/CODEX-BUILD-RUNBOOK.md | **Build order (linear):** `uv-dependency-groups-migration` → `tooling-cleanup` → `codex-model-adapter` → `composite-model-routing` → `brain-sensitivity-routing`. All WSL2/Windows-buildable; tests use fakes (no Ollama needed to build). Coder = **Codex CLI** on the ChatGPT subscription (`codex --sandbox workspace-write -m gpt-5.5`), driven per-spec from the runbook — OUTSIDE apex-code orchestration (owner supervises). `cross_model_review` on `brain-sensitivity-routing` now **SATISFIED** (Claude planned/reviewed → GPT builds = cross-family). Prereq: `codex login`. Starts from last green (121 tests @ validation slice). | — |
