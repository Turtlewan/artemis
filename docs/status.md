...
| cross-module reactions (surface 7) | planning | 🟡 IN PROGRESS — behavior-first triage | docs/owner-rules/7-cross-module-reactions.md + docs/findings/cross-module-io-map.md | **Designing the cross-module "when X → then Y" automation-rule (reaction) LAYER** — the flagged cross-module-linking ADR. Mapped every module's I/O surface (6 parallel agents → `cross-module-io-map.md`). **KEY FINDING: no reaction infrastructure exists** (request-response + time-polled only); needs 3 new pieces (emit events · rule store · reaction dispatcher) built over existing parts (ToolRegistry · entity backbone · GATE · quarantine · recipe loop). Owner chose **learned-first** direction (suggest→confirm→graduate) — not yet locked. **Cluster A (email-triggered) triaged + DONE**; A5 (flight) + A7 (interview, candidate) + A8 (Ashley partner-CRM) expanded into multi-step PLAYBOOKS. **New capabilities surfaced:** Trip aggregator (TripIt-style), "share/clip to Artemis" iOS channel, gift-signal/wishlist capture, bill reconciliation (A9), `transaction.instrument` field. **NEXT: triage clusters B–E** (Finance/Tasks/Calendar/cross-cutting), then lock the reaction approach → ADR. | docs/owner-rules/7-*.md + docs/findings/cross-module-io-map.md (uncommitted) |
