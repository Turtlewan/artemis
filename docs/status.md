...
| architecture-validation reservations | planning | 📋 DRAFT — A–H decided · rung-3/voice/I/J open · **edits HELD** | docs/drafts/architecture-validation-reservations.md | Walking the arch-validation research (reports 01–04) reservations + deciding each (owner: decide ALL before editing). **Decided:** A–E reserve cheap schema hooks (derived-provenance · non-triple memory port · async/scope port terms · RAPTOR summary-tree fields · structured-projection ingest hook); **F** = checkpoint+replay shared durable-exec + idempotency convention; **G** = first-class router→planner escalation seam (v1 via Task Executor); **H** = fallback ladder **Codex → DeepSeek-Pro-API → local** + recipe-quality gate/re-seed. **RESUME = finish H rung-3 local model (Qwen3-Instruct vs R1-distill-Qwen vs decide-at-Mac) → voice/portfolio-fit-on-64GB → decide I (parametric-stance doc line) + J (prospective-memory home) → then APPLY across M3-a · M4-a · ADR-004 · ADR-024 · M1 · M6 · M7 · ADR-022/027 · brain.md.** Full decision log in the draft. Report `04-seven-memory-types.md` committed. | docs/drafts/architecture-validation-reservations.md |
