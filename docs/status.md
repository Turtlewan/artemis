...
- **`SubprocessSandbox` is interim** (timed host subprocess, no isolation). The WSL2-isolated runner (no-network + egress allowlist + resource caps) is **required before any *externally*-authored capability is trusted** — swaps in behind the `SandboxRunner` protocol. Security gate, not optional. **Now scheduled under ADR-035** (the enabler stack); the **WSL2 isolation spike is the next build step**. Host env-probe (2026-07-01) confirms unprivileged netns + cgroups v2 + systemd present; only `iptables` is missing (the egress-allowlist gap).
