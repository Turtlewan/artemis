...
| cross-module reactions (surface 7) | planning | 🟡 IN PROGRESS — behavior-first triage (context cleared mid-triage) | docs/owner-rules/7-cross-module-reactions.md + docs/findings/cross-module-io-map.md | **Designing the cross-module "when X → then Y" reaction LAYER** (the flagged cross-module-linking ADR). Mapped every module's I/O (6 agents → `cross-module-io-map.md`). **KEY FINDING: no reaction infra today** (request-response + time-polled only); needs 3 new pieces (emit events · rule store · reaction dispatcher) over existing parts (ToolRegistry · entity backbone · GATE · quarantine · recipe loop). Approach leaning **learned-first** (suggest→confirm→graduate) — NOT locked. **Done:** cluster A (email) triaged; A5 flight / A7 interview-candidate / A8 Ashley-partner-CRM expanded into multi-step PLAYBOOKS. **B/C/E permutations enumerated — owner triage of ⭐ items PENDING; D not triaged.** New capabilities surfaced: Trip aggregator · "share/clip to Artemis" iOS channel · gift-signal wishlist · bill reconciliation (A9) · `transaction.instrument`. **▶ RESUME: triage B/C/E ⭐ (esp. deep-dive B4c charge↔receipt+fraud, E8 what's-due-this-week) → cluster D → lock approach → cross-module-reaction ADR.** See workbook §RESUME HERE. | ✅ 69729f0 + session-end commit |
