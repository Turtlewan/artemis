...
- [ ] Task 2: Define the retrieval ports — files: `/Users/artemis-build/artemis/src/artemis/ports/retrieval.py` — four `Protocol`s, methods bodies = `...` only:
  - `EmbeddingModel`: `async def embed_documents(self, texts: Sequence[str]) -> list[Vector]: ...` (to-be-STORED text — docs, chunks, facts, tool descriptions; NO instruction prefix); `async def embed_query(self, query: str) -> Vector: ...` (SEARCH text — a single query; the adapter applies the model's query instruction prefix here; note the deliberate shape: single `str` in, single `Vector` out, not a list — embed queries one at a time so the query path is unmistakable); `@property def dimension(self) -> int: ...` (dimension locked in store metadata per brain.md — exposed read-only; `dimension` is a cached int, stays sync).
  ...
  - **ASYNC PORT RULE (ADR-015):** a port method that performs **network I/O** (LLM / embedding / rerank calls to the model server) is `async def`; a method that only touches **local disk / SQLCipher / returns a cached value** stays sync. In this package that makes async: `EmbeddingModel.embed_documents`, `EmbeddingModel.embed_query`, `Reranker.rerank`, `Retriever.retrieve`, `ModelPort.complete` (already), `ModelPort.embed`, and `MemoryStore.{recall, inject_context, add_fact, update_fact}` (they embed). Sync: `VectorStore.*`, `EmbeddingModel.dimension`, `MemoryStore.delete_fact` (tombstone, no embed), `Router.route`, all voice ports.
