...
- [ ] Task 6: Implement the pipeline with content_hash idempotency — files: `/Users/artemis-build/artemis/src/artemis/ingest/pipeline.py` — `class IngestPipeline` constructed with `(connector_for: Callable[[Source], Connector], parser: DocumentParser, embedder: EmbeddingModel, store_for: Callable[[Scope], VectorStore], is_unlocked: Callable[[], bool])`. The `connector_for(source) -> Connector` argument dispatches by `source.kind` — callers wire connectors at the composition root (e.g. `{"file": FileConnector(...), "web": WebConnector()}.get`). Name the wiring explicitly when constructing `IngestPipeline` at the composition root (Seam 8 connector registration fix). `async def ingest(self, source: Source) -> IngestResult` (async per ADR-015 — the embed step awaits the async `EmbeddingModel.embed`) where `IngestResult` = `{ document_id: str, chunks_written: int, skipped: bool, parsed: ParsedDocument, document: Document, item: RawItem }` (the `parsed`/`document`/`item` fields expose the step-2 locals so a subclass — M3-d's `VisualIngestPipeline` — can run the visual stage off the same parse without re-running the connector; import `ParsedDocument` from `artemis.ingest.parsing`, `Document`/`RawItem` from the relevant ingest modules):
  ...
  Degrade-don't-crash: wrap connector/parse failures in a typed `IngestError` (never a bare exception out of `ingest` for a single bad source). — done when: `uv run mypy --strict src` passes; (in Task 9 test) `await`ing `ingest` on the same source twice writes chunks once then returns `skipped=True`.
