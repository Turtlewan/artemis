...
- [ ] Task 6: Implement the pipeline with content_hash idempotency — files: `/Users/artemis-build/artemis/src/artemis/ingest/pipeline.py` — `class IngestPipeline` constructed with `(connector_for: Callable[[Source], Connector], parser: DocumentParser, embedder: EmbeddingModel, store_for: Callable[[Scope], VectorStore], is_unlocked: Callable[[], bool])`. The `connector_for(source) -> Connector` argument dispatches by `source.kind` — callers wire connectors at the composition root (e.g. `{"file": FileConnector(...), "web": WebConnector()}.get`). Name the wiring explicitly when constructing `IngestPipeline` at the composition root (Seam 8 connector registration fix). `async def ingest(self, source: Source) -> IngestResult` (async per ADR-015 — the embed step awaits the async `EmbeddingModel.embed`) where `IngestResult` = `{ document_id: str, chunks_written: int, skipped: bool, parsed: ParsedDocument, document: Document, item: RawItem }` (the `parsed`/`document`/`item` fields expose the step-2 locals so a subclass — M3-d's `VisualIngestPipeline` — can run the visual stage off the same parse without re-running the connector; import `ParsedDocument` from `artemis.ingest.parsing`, `Document`/`RawItem` from the relevant ingest modules):
  ...
  5. `chunks = chunk_document(parsed, doc)`; `vectors = await embedder.embed([c.text for c in chunks])` (the late-chunking embed step; `EmbeddingModel.embed` is `async def` per ADR-015 — `await` it; document that a true long-context late-chunk pooling is the embedder adapter's concern, the port stays `async def embed(texts)`); assert `len(vectors)==len(chunks)`.
...
- [ ] Task 9: Write the off-hardware ingest tests — files: `/Users/artemis-build/artemis/tests/test_ingest_pipeline.py` — typed pytest with `FakeEmbedder` (deterministic fixed-dimension vectors; `async def embed(self, texts) -> list[Vector]` per ADR-015 — `.dimension` stays a sync property), `FakeParser`, a `FileConnector` over a temp file + a `WebConnector` with an injected fixture HTML, and a real `LanceDBVectorStore` against a temp dir (set `ARTEMIS_DATA_ROOT` to a tmp path so `vault_dir` resolves under it, `is_unlocked=lambda:True`; D4: no `ARTEMIS_VOLUME_ROOT` setting needed). NOTE: `ingest` is now `async def` — the ingest tests are `async def test_*` under `@pytest.mark.asyncio` (no prior async-test convention in this spec; add `pytest-asyncio` if not already a dev dep) and `await pipeline.ingest(...)`; the sync `LanceDBVectorStore` methods (`add`/`search`/`has_document`/`delete_document`) are called WITHOUT `await` (VectorStore stays sync per ADR-015):
