...
- Google `calendar.readonly` scope covers `calendarList.list`, `events.list`, `events.get`, and the FreeBusy API; `calendar.events` scope is required for FreeBusy on the owner's own calendars per Google's API docs. Both scopes are registered by CAL-a. → impact: Caution (if the scope set proves insufficient at on-hardware testing, expand in CAL-a's `register_google_scopes` call).
...
- [ ] Task 4: §A read tools + `FindTimeEngine` — files: `/Users/artemis-build/artemis/src/artemis/modules/calendar/read_tools.py` —
  ...
  **Callable functions** — all take `(args, *, store: EventCacheStore, prefs: CalPrefs, client: CalendarClient | None = None)` as injected dependencies (NOT as `ToolSpec.args_schema` fields — the brain passes `args`; the module wires the store/prefs at dispatch time via `functools.partial` or a `CalendarTools` class constructed with the dependencies; document this dispatch pattern clearly):
  ...
  - `list_calendars(args: ListCalendarsArgs, *, client: CalendarClient) -> ListCalendarsResult`: calls `client.list_calendars()` (the live calendar list is not cached, as it rarely changes and is lightweight). Parse each item to `CalendarInfo`.

  - `list_events(args: ListEventsArgs, *, store: EventCacheStore) -> ListEventsResult`: `store.query_events(calendar_ids=args.calendar_ids, time_min=args.window.start, time_max=args.window.end)`; if `args.query` apply a case-insensitive substring filter on `summary`. Map to `EventSummary` list.

  - `get_event(args: GetEventArgs, *, store: EventCacheStore) -> GetEventResult`: `store.query_events(...)` filtered by `event_id` and `calendar_id`; return the first match as `EventDetail` or `event=None`.
  ...
  - `next_event(args: NextEventArgs, *, store: EventCacheStore, prefs: CalPrefs) -> NextEventResult`: query from now → `now + sync_window_months_future`; exclude overlays and cancelled; return the earliest upcoming event.
  ...
  - `free_busy(args: FreeBusyArgs, *, store: EventCacheStore) -> FreeBusyResult`: query cache for the window; build busy blocks from non-cancelled, non-overlay events (overlay projections are own-holds, not real meetings — exclude them from free/busy calculation to avoid self-blocking). Sort by `start_dt`.
  ...
  - **`FindTimeEngine`** — a class constructed with `(prefs: CalPrefs)`, holding the deterministic slot-finding logic (pure, no I/O). Used by both `find_time` and `find_time_with_attendees`.
    ...
    `def find_slots(self, busy_blocks: list[tuple[datetime, datetime]], window_start: datetime, window_end: datetime, duration_minutes: int, *, buffer_minutes: int | None = None) -> list[FreeSlot]`: pure algorithm.
    ...
    `def find_time_tool(args: FindTimeArgs, *, store: EventCacheStore, prefs: CalPrefs) -> FindTimeResult`: query free_busy (step in this file); build `FindTimeEngine(prefs).find_slots(busy, window, args.duration_minutes, buffer_minutes=args.buffer_minutes)`.

    `def find_time_with_attendees_tool(args: FindTimeWithAttendeesArgs, *, store: EventCacheStore, prefs: CalPrefs, client: CalendarClient) -> FindTimeWithAttendeesResult`:
  ...
  - `conflicts(args: ConflictsArgs, *, store: EventCacheStore, prefs: CalPrefs) -> ConflictsResult`: default range = today + 7 days in `prefs.timezone`; query events; find overlapping pairs (O(n²) for small n); group overlapping clusters; return `ConflictGroup` per cluster of ≥2 overlapping events.
...
- [ ] Task 5: Module manifest + scope registration — files: `/Users/artemis-build/artemis/src/artemis/modules/calendar/manifest.py`, `/Users/artemis-build/artemis/src/artemis/modules/calendar/__init__.py` —
  ...
  class CalendarTools:
      ...
      def list_calendars(self, args: ListCalendarsArgs) -> ListCalendarsResult: ...
      def list_events(self, args: ListEventsArgs) -> ListEventsResult: ...
      # ... all §A callables
  ```
  Each method calls the corresponding function from `read_tools.py` with the injected deps.
  ...
  In `__init__.py`: register scopes at module import time:
  ```python
...
- [ ] Task 7: Off-hardware tests (fakes only) — files: `/Users/artemis-build/artemis/tests/test_calendar_read.py` —
  ...
  Typed pytest. Use `FakeCalendarClient`, `FakeKeyProvider` (from M2-b), in-memory shims for stores (define `FakePreferencesStore` and `FakeCacheStore` as simple dict-backed implementations of the same interface, NOT using SQLCipher — used for all functional tests; the real SQLCipher round-trips are gated in Task 8).
...
- [ ] Run `uv run python -c "from artemis.integrations.google.scopes import required_scopes; print('calendar.readonly' in ' '.join(required_scopes()))"` → verify: prints `True` (calendar scopes registered on calendar module import).
