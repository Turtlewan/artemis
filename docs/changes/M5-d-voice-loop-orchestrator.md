...
- [ ] Task 3: Implement the VoiceLoop orchestrator — files: `/Users/artemis-build/artemis/src/artemis/voice/voice_loop.py` — `class VoiceLoop` constructed with `(frontend: AudioFrontend, stt: STT, tts: TTS, gateway: Gateway, settings, ack_pcm: bytes, budget: LatencyBudget)`. The cascade (an async loop):
  ...
  5. speaker-ID + scope + Tier nuance: call `gateway.handle_voice(utterance_pcm, transcript)` — BUT to stream, use the streaming voice path (the M5-c `handle_voice` returns a `BrainResponse`; for streaming, the loop calls the streaming variant — `gateway.handle_voice_stream(utterance_pcm, transcript) -> AsyncIterator[str]` (added in Task 4, consuming `Brain.respond_stream`)). `mark("speaker_id_done")` after identity resolves. If the gateway returns `NEEDS_PHONE_UNLOCK` (locked owner Tier-1, M5-c) → synth + play that prompt and end the turn (do NOT serve sensitive data).
...
- [ ] Task 4: Wire the voice composition + the streaming brain/gateway seam — files: `/Users/artemis-build/artemis/src/artemis/gateway.py` (modify), `/Users/artemis-build/artemis/src/artemis/voice/voice_loop.py` (the composition helper) — add `def compose_voice_loop(settings) -> VoiceLoop`: build the `SidecarAudioFrontend` (socket path from paths), `ParakeetWhisperSTT`/`KokoroTTS` (M5-b) + `warmup_all`, the `Gateway` from M5-c's `compose_brain` (with the real `SpeakerID`/`KeyProvider`), load the cached `ack_pcm` (the drafted default: a pre-rendered bundled earcon file loaded at startup; a synth-once spoken filler is the config swap), a `LatencyBudget` from config, and return the `VoiceLoop`. In `gateway.py` add the streaming voice entry `handle_voice_stream(audio, transcript) -> AsyncIterator[str]` consuming the `Brain.respond_stream` M1-b back-fill, and reusing the `Brain.pre_route` Tier gate from M5-c — keep `handle_voice`/`handle_text` from M5-c/M2-b intact. — done when: `uv run mypy --strict src` passes; `compose_voice_loop(get_settings())` returns a `VoiceLoop` without contacting the sidecar or loading models at construction (lazy; warmup is explicit).
...
- [ ] Task 5: Write the voice-loop tests — files: `/Users/artemis-build/artemis/tests/test_voice_loop.py` — typed pytest, all off-hardware with `FakeSidecar` + `FakeSTT`/`FakeTTS`/`FakeSpeakerID`/`FakeBrain`/`FakeKeyProvider`:
  ...
  - voice scope nuance (delegates to M5-c): OWNER + Tier-1 + locked → the loop plays a `NEEDS_PHONE_UNLOCK` prompt and serves NO sensitive answer; OWNER + Tier-0 + locked → proceeds; UNKNOWN → guest scope used (FakeBrain asserts the guest scope).
