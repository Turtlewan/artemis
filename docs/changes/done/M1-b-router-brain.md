...
- **Reservation note (architecture-validation 2026-06-23, reservation G — planner escalation seam; ADR-024 Refinement 2026-06-23):** the `path="escalate"` outcome is the reserved seam for the future **`router → planner` long-horizon escalation tier** (a peer to the router, realized via the Task Executor at M9). Keep `RouteDecision`'s `path` an **open-set discriminant** (don't hard-code consumers to assume exactly the three v1 values) so a future `path="plan"` / planner-target route can be added without re-shaping the port. M1 builds only the stub; this note just prevents narrowing the seam. → impact: Low (no v1 behaviour change; documents the reserved escalation tier).
