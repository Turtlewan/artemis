<!-- amended 2026-06-11 per contracts.md (Seam 1) + m2-obs-dr-security.md FLAGs F1; TracingModelPort.complete async; sqlcipher_open key type fix -->
---
...
- `ModelPort.complete` is **`async def complete(role, messages, *, stream, response_schema, temperature, max_tokens) -> ModelResponse`** per contracts.md Seam 1; `ModelResponse{text, finish_reason, usage, origin, model_id}` (M0-d + Seam 1 Δ). `TracingModelPort` must also be `async def complete` and `await inner.complete(...)` (Seam 1). The adapter is responsible for normalising provider usage to `ModelResponse.usage` fields; the tracer reads them defensively (None → `{}`; missing key → 0; `total_tokens` absent → `prompt+completion`). A fully-zero usage logs a WARNING (a visible tracing gap, e.g. a streaming call that omitted usage), never a crash. Anthropic prompt-cache tokens (if surfaced) are NOT separately subtracted in v1 → subscription token totals are a conservative upper bound (documented). → impact: Stop (resolved).
...
- [ ] Task 1: The SQLCipher telemetry store — files: `/Users/artemis-build/artemis/src/artemis/obs/telemetry/store.py` —
  ...
  - `def open_telemetry_db(path: Path, key: SecretKey) -> Connection`: ensure parent dir exists; **(resolves F1)** open keyed via M2-c `sqlcipher_open(path, key.as_hex())` — `sqlcipher_open` takes `key_hex: str`, NOT a `SecretKey` object; pass `key.as_hex()` (NOT plain sqlite3 — the store is born encrypted); `PRAGMA journal_mode=WAL`; `PRAGMA synchronous=NORMAL`; row factory rows-as-tuples. (Single factory so the keyed-open is localized.)
...
- [ ] Task 3: The `TracingModelPort` wrapper — files: `/Users/artemis-build/artemis/src/artemis/obs/telemetry/tracing.py` —
  ...
  - **(Seam 1 — contracts.md)** `async def complete(self, role, messages, *, stream=False, response_schema=None, temperature=0.7, max_tokens=None) -> ModelResponse`: record `t0 = time.perf_counter()`; `resp = await inner.complete(role, messages, stream=stream, response_schema=response_schema, temperature=temperature, max_tokens=max_tokens)`; `latency_ms = int((time.perf_counter() - t0) * 1000)`; `usage = getattr(resp, "usage", None)` (`ModelResponse.usage` is the M0-d frozen `Usage` dataclass `{prompt_tokens: int, completion_tokens: int, total_tokens: int}` — an OBJECT, NOT a dict; read fields via `getattr`, never `.get()`; None/absent → all-zero, never crashes); `prompt = int(getattr(usage, "prompt_tokens", 0) or 0)`, `completion = int(getattr(usage, "completion_tokens", 0) or 0)`, `total = int(getattr(usage, "total_tokens", 0) or 0) or (prompt + completion)`; if `prompt == completion == total == 0` → `get_logger("obs.tracing").warning("empty_usage", extra={"role": role[:64]})` (visible tracing gap, e.g. streaming without usage); `model_id = settings.roles[role].model_id if role in settings.roles else None`; `cost_micros = cost.cost_micros(role, total)`; build `CallTrace(role[:64], model_id, prompt, completion, total, latency_ms, cost_micros, trace_id=None, at=self._clock())`; `store.record_call(trace)` wrapped in try/except → on failure `get_logger("obs.tracing").warning("record_call_failed", extra={"role": role[:64], "error_type": type(exc).__name__})` (NEVER `messages`/`resp.text`/`str(exc)`) and still return `resp`. Return the unchanged `resp`.
...
<!-- LINT-DEFER 2026-06-11: verified_at timestamp format unspecified — M7-a1 declares provenance: dict[str,str] but does NOT pin the verified_at encoding (ISO-8601 vs epoch). Pinning a parser here (e.g. datetime.fromisoformat) risks mismatching M7-a1's real format. Needs M7-a1 to declare the format first. -->
...
- [ ] Task 6: Tests — files: `/Users/artemis-build/artemis/tests/test_obs_telemetry.py` — typed pytest over `tmp_path` (a keyed SQLCipher DB via `open_telemetry_db(tmp_path/"t.db", TEST_KEY)` mirroring M4-a), reusing M0/M7 fakes (`FakeModelPort`, a real `RecipeStore`, a fixed test `Settings`):
  ...
  - tracer: static `_check: ModelPort = TracingModelPort(...)`; `complete` with `usage=Usage(prompt_tokens=0, completion_tokens=0, total_tokens=42)` (the M0-d `Usage` dataclass — NOT a dict) → one row, `total_tokens==42`, `model_id` from settings; `usage=None` → zero-token row + WARNING + no raise; a store raising on `record_call` → `complete` still returns.
  ...
  - end-to-end M7-c shape: `scan_gaps(SqliteTelemetrySource(...), now=...)` (from `artemis.curiosity.gaps`; signature `def scan_gaps(telemetry: TelemetrySource, *, now: datetime, confidence_floor: float = 0.5, staleness_days: int = 90) -> list[Gap]`, where `Gap` is the M7-c frozen dataclass `Gap { task_class_key: str, kind: Literal["escalation-cluster","low-confidence","recurring-topic","staleness"], score: float, evidence_count: int }`) after 3 escalations sharing a key → a `Gap` with `.kind == "escalation-cluster"` and `.evidence_count == 3`.
