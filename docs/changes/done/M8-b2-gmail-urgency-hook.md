<!-- amended 2026-06-11 per contracts.md (Seams 7,8) + m8-gmail.md BLOCKs: B1 (SIGNAL_CATEGORIES import fix), B4/Seam 7 (drop raw subject/snippet from UrgencyCandidate payload), B5 (UrgencyTemplateRenderer registration wiring), B6 (sender display-name-only extraction specified), F4 (Files-to-Change signature corrected); Decision D2 (urgency candidate set = {PRIMARY, UPDATES}) -->
---
...
## Assumptions
- **M8-b1** complete: `artemis.modules.gmail` exports `GmailReadCache` (with `list_unread(category: MailCategory | None) -> list[CachedMessage]`), `CachedMessage` (fields: `message_id`, `sender`, `unread`, `important`, `category`, `snippet`, `subject`, `internal_date_ms`), `GmailClient`, `MailCategory`, `SIGNAL_CATEGORIES` (module-level `frozenset` constant in `client.py`, re-exported from `__init__.py` — **NOT** a `MailCategory` class attribute; import as `from artemis.modules.gmail import SIGNAL_CATEGORIES`), `FakeGmailApi`, `build_gmail_manifest`. M8-b1's manifest ships `proactive_hooks=[]`; M8-b2 replaces that call with one that passes the hook. → impact: Stop (M8-b2 adds the hook to the manifest; M8-b1's `build_gmail_manifest` signature must accept a `hook: HookSpec | None = None` param OR M8-b2 defines its own `build_gmail_manifest_with_hook` factory — see Task 4).
  **`URGENCY_CANDIDATES`**: M8-b2 defines `URGENCY_CANDIDATES = frozenset({MailCategory.PRIMARY, MailCategory.UPDATES})` in `urgency.py`. Stage-1 of the urgency pre-filter uses `URGENCY_CANDIDATES`, NOT `SIGNAL_CATEGORIES` (which is a broader set that also includes `FORUMS`). `SIGNAL_CATEGORIES` is unchanged and continues to serve its other uses (ingestion depth, memory extraction). Only the urgency candidate filter narrows to `{PRIMARY, UPDATES}`.
...
| /Users/artemis-build/artemis/src/artemis/modules/gmail/hook.py | create | `build_known_senders(memory, person_id, k) -> frozenset[str]` (async, composition-time only) + `build_gmail_urgency_hook(cache, api, reader, known_senders, *, max_candidates, interval_seconds) -> tuple[HookSpec, Callable]` — the `HookSpec` factory; the LLM-free `check_ref` closure. **Task 3 signature is canonical** (F4 fix: the Files-to-Change table previously omitted `reader` and the keyword params). |
...
- [ ] Task 1: `UrgencyPayload` type + `GmailUrgencyPreFilter` (Stages 1+2) — files: `/Users/artemis-build/artemis/src/artemis/modules/gmail/urgency.py` —
  ...
  class UrgencyCandidate:
      message_id: str
      sender: str          # display name ONLY — extracted via email.utils.parseaddr(msg.sender)[0];
                           # if parseaddr returns empty, fall back to msg.sender.split("@")[0].
                           # NEVER a raw From: address or full header in the payload (B6/Seam 7 fix).
                           # Truncate to 100 chars after extraction.
  ...
  ```
  **`subject` and `snippet` are REMOVED from `UrgencyCandidate`** (Seam 7 / B4 fix: subject and snippet are attacker-controlled free text — the canonical injection vector per ADR-009. The spec's earlier claim that M6-b's `<<<...>>>` wrapping is sufficient protection is false: subject/snippet are not quarantined by DR-a. The `Extract.summary` already covers content for urgency scoring; raw subject/snippet must not reach the privileged model or the hook payload. This resolves the direct contradiction with the Security section's own "ONLY ever sees the DR-a Extract — NEVER raw mail bodies" invariant.)
  ...
  - `def stage1_candidates(self) -> list[CachedMessage]`: call `cache.list_unread(category=None)` → keep only messages where `msg.unread is True AND msg.important is True AND msg.category in URGENCY_CANDIDATES`. `URGENCY_CANDIDATES = frozenset({MailCategory.PRIMARY, MailCategory.UPDATES})` — defined at module level in `urgency.py` (Decision D2: Forums excluded from urgency scoring; `SIGNAL_CATEGORIES` is broader and used elsewhere for ingestion depth). Note: `SIGNAL_CATEGORIES` is a module-level `frozenset` constant in `client.py` (NOT a `MailCategory` class attribute; `MailCategory.SIGNAL_CATEGORIES` does not exist and raises `AttributeError` — B1 fix applies there). Sort by `internal_date_ms` descending. Return the top `self.max_candidates`. If the result is empty, return `[]`.
  ...
  - `def build_payload(self, boosted: list[tuple[CachedMessage, bool]], extracts: dict[str, Extract]) -> dict[str, object]`: for each `(msg, known)`, build an `UrgencyCandidate` dict: `sender` = display-name extracted via `email.utils.parseaddr(msg.sender)[0]` (stdlib, no new dep); if the result is empty or whitespace, fall back to `msg.sender.split("@")[0]`; truncate to 100 chars — **this is the ONLY sender value in the payload; the raw `From:` header and full email address are NEVER included** (B6/Seam 7 fix); `known_to_memory = known`; look up `extracts.get(msg.message_id)` → if present and `not ex.parse_failed`: `extract_summary = ex.summary[:500]`, `extract_failed = False`; else `extract_summary = ""`, `extract_failed = True`. **Do NOT include `subject` or `snippet` in the candidate dict** (Seam 7: these are raw attacker-controlled fields; the Extract summary covers content). Return `{"candidates": [<dicts>], "unread_count": len(boosted)}`. NEVER include raw message body bytes, subject, snippet, the full `From:` header in the payload.

  `class UrgencyTemplateRenderer`:
  - `def render(self, result: HookResult) -> str`: the M6-b fallback template (registered in `TemplateRegistry` under `"gmail.gmail_urgency_check"`). Extracts `payload["candidates"]` and `payload.get("unread_count", 0)`. Returns a plain string: `f"{unread_count} unread important message(s): " + "; ".join(c['sender'] for c in candidates[:3])` (max 3 display names only — **no `subject` field; that field was removed from the payload per Seam 7**; the fallback carries sender names only, which is sufficient for a vault-locked degraded notification). If `candidates` is empty, returns `"No urgent unread messages."`. This is the NO-LLM fallback path — it deliberately omits urgency scoring.

  — done when: `uv run mypy --strict src` passes; `GmailUrgencyPreFilter(cache, known_senders=frozenset()).stage1_candidates()` over a `FakeGmailApi`-backed cache with mixed unread/important flags returns only `unread=True AND important=True AND category in URGENCY_CANDIDATES` messages (i.e. PRIMARY and UPDATES, not FORUMS); `stage2_boost` returns `known_to_memory=False` for all when `known_senders=frozenset()`; `UrgencyTemplateRenderer().render(HookResult.of({"candidates": [], "unread_count": 0}))` returns `"No urgent unread messages."` (Task 5).
...
- [ ] Task 3: `build_known_senders` async helper + `build_gmail_urgency_hook` — files: `/Users/artemis-build/artemis/src/artemis/modules/gmail/hook.py` —
  ...
  **`UrgencyTemplateRenderer` registration (B5 fix):** `build_gmail_urgency_hook` also returns the registration callable. Add a third element to the return tuple:
  ```python
  renderer = UrgencyTemplateRenderer()
  return hook, _pre_flight, lambda registry: registry.register("gmail.gmail_urgency_check", renderer.render)
  ```
  Update the return type annotation to `tuple[HookSpec, Callable[[], Awaitable[None]], Callable[[TemplateRegistry], None]]`. The caller (M6-c `compose_proactive`) MUST call the third element with the `TemplateRegistry` instance before the first tick — e.g. `hook, pre_flight, register_template = build_gmail_urgency_hook(...); register_template(template_registry)`. Without this call, M6-b falls back to a payload-free default string on model failure. Document this requirement in `hook.py`'s module docstring and in the composition-root comment.
  ...
  — done when: `uv run mypy --strict src` passes; `build_gmail_urgency_hook(cache, api, reader, known_senders=frozenset())` returns a three-element tuple `(HookSpec, Callable, Callable[[TemplateRegistry], None])`; the returned `HookSpec` has `needs_llm=True`, `tier=1`, `urgency="high"`, `dedup_key="gmail_urgency"`; `hook.check_ref()` returns `HookResult.miss()` when the cache has no unread-important messages; returns a `HookResult` with `hit=True` and a non-empty `payload["candidates"]` list when the cache has matching messages; each candidate dict has keys `message_id`, `sender`, `known_to_memory`, `extract_summary`, `extract_failed` — **no `subject` or `snippet` keys** (Seam 7) (Task 5).
...
- [ ] Task 5: Off-hardware tests — files: `/Users/artemis-build/artemis/tests/test_gmail_urgency_hook.py` — typed pytest with `FakeGmailApi`, a real `GmailReadCache` over a temp SQLCipher DB (via `FakeKeyProvider(owner_unlocked=True)`), a `FakeModelPort`-backed `QuarantinedReader`. No `FakeMemoryStore` needed — Stage 2 is now a pure frozenset substring check with no memory calls inside `check_ref`. Tests for `build_known_senders` use a `FakeMemoryStore` (implementing `MemoryStore` Protocol) only for the async builder helper tests. `FakeMemoryStore.recall` MUST be `async def` (`MemoryStore.recall` is async per ADR-015); the helper-test bullets below `await build_known_senders(...)`, which transitively awaits `recall`.

  - **Stage 1 — pre-filter logic:** seed the cache with 6 messages: (a) `unread=True, important=True, category=PRIMARY` → INCLUDED; (b) `unread=False, important=True, category=PRIMARY` → excluded (not unread); (c) `unread=True, important=False, category=PRIMARY` → excluded (not important); (d) `unread=True, important=True, category=PROMOTIONS` → excluded (not in `URGENCY_CANDIDATES`); (e) `unread=True, important=True, category=UPDATES` → INCLUDED; (f) `unread=True, important=True, category=FORUMS` → excluded (not in `URGENCY_CANDIDATES` — Decision D2). Assert `stage1_candidates()` returns exactly 2 messages (a and e) sorted newest-first. Assert message (f) is NOT returned (FORUMS is in `SIGNAL_CATEGORIES` but not `URGENCY_CANDIDATES`).
  ...
  - **Payload shape:** seed 2 unread-important-Primary messages (with a `sender` value like `"Alice Smith <alice@example.com>"`); unpack `hook, pre_flight_factory, register_template = build_gmail_urgency_hook(cache, api, reader, known_senders=frozenset())`; call `await pre_flight_factory()` (event loop); then `result = hook.check_ref()`. Assert `result.hit is True`; `result.payload["candidates"]` has 2 entries; each entry has keys `message_id`, `sender`, `known_to_memory`, `extract_summary`, `extract_failed` — **assert `"subject"` NOT in entry and `"snippet"` NOT in entry** (Seam 7 invariant); assert `entry["sender"]` == `"Alice Smith"` (display name extracted via `parseaddr`, NOT the full `From:` header); `result.payload["unread_count"] == 2`; `result.dedup_value == date.today().isoformat()`.
  ...
  - **Template fallback renders:** `UrgencyTemplateRenderer().render(HookResult.of({"candidates": [{"sender": "Alice", "known_to_memory": False, "extract_summary": "", "extract_failed": True, "message_id": "x"}], "unread_count": 1}))` returns a non-empty string containing "Alice" (**no `subject` key in the candidate — assert the renderer does not reference `c["subject"]`**).
...
- The `UrgencyPayload` includes only display name (extracted via `parseaddr`, truncated to 100 chars) and the Extract summary (500 chars) — **no raw subject, no snippet, no body**. Subject and snippet were removed from the payload (Seam 7 amendment 2026-06-11).
- Sender display name is extracted via `email.utils.parseaddr` before entering the payload; it rides inside M6-b's `<<<...>>>` boundary.
...
- [ ] Run `uv run pytest -q tests/test_gmail_urgency_hook.py` → verify: Stage 1 filters to unread+important+`URGENCY_CANDIDATES` only (`{PRIMARY, UPDATES}` — FORUMS excluded despite being in `SIGNAL_CATEGORIES`); Stage 2 returns `known_to_memory=True` for a sender token in `known_senders` and `False` when `known_senders=frozenset()`; `build_known_senders` returns lowercased subject+object tokens and degrades to `frozenset()` on recall failure; empty inbox → `HookResult.miss()`; full payload shape (candidates list with no `subject`/`snippet` keys, unread_count, dedup_value=today); `sender` in payload is display name only (not raw `From:` header); extract summary populated when pre-flight succeeds; extract_failed=True on model failure; hit=True even when extract fails (Stage 1 drives the hit); template renders non-empty string with sender name only (no subject key); template renders "No urgent unread messages." for empty candidates; `build_gmail_urgency_hook` returns a 3-tuple (hook, pre_flight_factory, register_template); `register_template(registry)` registers the renderer; manifest with hook has 1 hook with needs_llm=True, tier=1; manifest validator passes — all pass.
