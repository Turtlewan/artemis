...
|------|-----------|-------|
| /Users/artemis-build/artemis/swift/ArtemisApp/Sources/Screens/BrainApi.swift | create | app-local `BrainApi` protocol + `extension ApiClient: BrainApi`; a small `tokenedApi` helper binding the current token; **+ `protocol AppCoordinating { func lock() async throws; func logout() async }`** (ADR-017 — the screen→coordinator seam StatusModel depends on; lives in the shared `Sources/Screens/` dir so both the iOS + macOS targets compile it) |
...
- [ ] Task 4: Status screen — files: `/Users/artemis-build/artemis/swift/ArtemisApp/Sources/Screens/StatusScreen.swift` —
  - `@MainActor @Observable final class StatusModel` constructed with `(api: BrainApi, token:, coordinator: any AppCoordinating)` (**ADR-017 multiplatform: depends on the `AppCoordinating` protocol, NOT the concrete iOS `AppModel`, so the macOS `MacAppModel` (CLIENT-f) reuses this screen unchanged** — resolves CLIENT-f conformance-question #2). State: `status: StatusResponse?`, `loading`, `error`. `func refresh() async` (`api.status(token:)`); `func lockNow() async` (`await coordinator.lock()` — single lock path via the coordinator, B6/U4 fix; do NOT call `api.lock` directly and then write coordinator state manually); `func signOut() async` (`await coordinator.logout()`).
