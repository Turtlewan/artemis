...
|------|-----------|-------|
| src/artemis/api/oauth_routes.py | create | `APIRouter`: `POST /app/oauth/google/connect` (start flow → returns consent URL), `GET /app/oauth/google/status` (connected accounts + granted scopes), `POST /app/oauth/google/disconnect` |
| src/artemis/api/app.py | modify | `app.state.oauth_broker = OAuthBroker(app.state.secrets, open_browser=<no-op>)`; include the oauth router |
| tests/api/test_oauth_routes.py | create | session-gate + happy-path (mocked broker) + client-not-configured + disconnect |

## Tasks
- [ ] Task 1: Create `oauth_routes.py` with the three session-gated endpoints backed by an injected/`app.state` broker. `connect` accepts the requested scopes, calls `broker.begin_connect`, and returns the consent URL (client opens it) or a typed `client_not_configured` status when creds are absent — it never opens a browser server-side; `status` returns connected accounts + granted scopes (token values never included); `disconnect` calls `broker.disconnect`. — files: src/artemis/api/oauth_routes.py, tests/api/test_oauth_routes.py — done when: the three endpoints return their typed payloads, reject unauthenticated calls (session gate), and `connect` returns the consent URL without opening a browser in-process.
- [ ] Task 2: Wire the broker in `create_app` (`app.state.oauth_broker`, built from `app.state.secrets` with a no-op `open_browser`) and include the router. — files: src/artemis/api/app.py, tests/api/test_oauth_routes.py — done when: a `TestClient` reaches the routes and `app.state.oauth_broker` is present.
...
| Modify | src/artemis/api/app.py, CHANGELOG.md |
