...
- [ ] Run `uv run python -c "from artemis.memory import create_schema; import sqlite3; c=sqlite3.connect(':memory:'); create_schema(c, embedder_model_id='m', dimension=8); names={r[0] for r in c.execute(\"SELECT name FROM sqlite_master WHERE type='table'\")}; assert {'entities','entity_aliases'} <= names; cols={r[1] for r in c.execute('PRAGMA table_info(facts)')}; assert 'subject_entity_id' in cols; print('ok')"` → verify: prints `ok` (NOTE: this in-memory check works only if `create_schema` can run without the sqlite-vec extension; if M4-a's `create_schema` requires sqlite-vec loaded, run the equivalent assertions inside `tests/test_memory_entities.py` against the fixture connection instead and rely on the pytest gate below).
...
- [ ] Run `uv run python -c "from artemis.memory import EntityType, EntityRow, EntityRef, EntityRepository, person_fact_key; print(person_fact_key(external_ref='A@B.com', name='x') == person_fact_key(external_ref='a@b.com', name='y'))"` → verify: prints `True`.
