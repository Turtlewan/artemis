...
**COMPLETE (2026-07-01, Codex gpt-5.5 / host = Opus).** mypy 103 · 264 passed/3 skipped · ruff+format clean. 16 web_tool tests (quarantine invariant proven non-vacuously with a real injection payload; hard/soft/independent escalation; per-hit containment; egress catch+log domain-only; partial/zero-source; synth-degrade; sources=cited∩fed; faithfulness; search-failure degrade). Live-smoke = skipped marker.
- Cross-model review (Opus): no BLOCK. Fixed 3 — imported canonical `artemis.ports.model.ModelPort` (was locally redefined); `answer()` now degrades on search-provider failure (was unhandled); CHANGELOG entry added.
- **⚠️ Security follow-up (out of this spec's scope):** the reader "no-tools" quarantine is not structurally enforced at the `claude -p` CLI (`--allowedTools ""`) — see status.md Open Questions; gate before the reader goes live.
