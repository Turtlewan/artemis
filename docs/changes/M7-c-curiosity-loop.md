...
autonomy_level: L2
---
<!-- amended 2026-06-11 per contracts.md (Seam 1) + m7-cap-teacher-distill.md FLAG F7 -->
...
- Environment setup required: `uv add tldextract` (NEW runtime dep — eTLD+1 / registrable-domain helper for the grounding gate; bundles the Public Suffix List so `.co.uk`-class suffixes resolve). Off-hardware suite otherwise uses fakes (telemetry, researcher, reachability, model). The live research run (real `Researcher` + real reachability) is **GATED on-hardware** and depends on the real Deep-Research engine (a separate capability) — M7-c only proves the loop against the stub.

## Files to Change
| File | Operation | Notes |
|------|-----------|-------|
| /Users/artemis-build/artemis/src/artemis/curiosity/__init__.py | create | package marker + re-exports (`CuriosityLoop`, `Researcher`, `StubResearcher`, `TelemetrySource`, `registrable_domain`, `grounding_gate`, `TokenLedger`, `StagedItem`) |
...
| /Users/artemis-build/artemis/src/artemis/curiosity/research.py | create | `Researcher` Protocol + `ResearchResult`/`Source` types + `StubResearcher` + `registrable_domain` (eTLD+1 via `tldextract`) + `grounding_gate` (≥2 external reachable distinct-eTLD+1, not self-generated) |
...
- [ ] Task 2: Define the Researcher port + stub + the grounding gate — files: `/Users/artemis-build/artemis/src/artemis/curiosity/research.py` —
  ...
  - `def registrable_domain(url: str) -> str`: the eTLD+1 (registrable domain) of the URL's host via `tldextract.extract(url).registered_domain` (NEW runtime dep `tldextract` — it bundles the Public Suffix List, so `.co.uk`-class multi-part suffixes resolve correctly; a naive `host.split(".")[-2:]` is WRONG and is forbidden). This helper is **exported** so DR-b reuses it one-way (DR-b's `registrable_domain` reconciliation, see DR-b Assumptions). NO model call, NO network (PSL is bundled).
  - `def grounding_gate(result: ResearchResult, reachability: Reachability) -> bool`: accept iff `result.self_generated is False` AND there are **≥2 sources with distinct registrable domain (eTLD+1, computed via `registrable_domain(source.url)`** — NOT the raw `domain` string; two subdomains of one publisher are NOT independent) AND **≥2 of those distinct-domain sources are reachable** (a single transient-unreachable source does not discard an otherwise-grounded result). `reachability.is_reachable` counts only `2xx`/`3xx` as reachable (a `403`/`429` is "gated, not grounding-usable") and uses a short FIXED timeout (required — an un-timed HEAD/GET would hang the gate). Reject otherwise (the anti-collapse invariant). **`GroundingError` is defined for future use but NOT raised by `grounding_gate` — the gate returns `bool`.** NO model call.
  — done when: `uv run mypy --strict src` passes; `grounding_gate` returns True for a 2-distinct-domain reachable non-self-generated result, and False if `self_generated` or **fewer than 2 distinct-domain sources are reachable** (a transient single unreachable among ≥2 reachable-distinct-domain sources must still return True — "any source unreachable" was the wrong gate; the correct gate is "fewer than 2 distinct-domain reachable remain"; see F7 fix).

- [ ] Task 3: Implement the token caps ledger + staging store — files: `/Users/artemis-build/artemis/src/artemis/curiosity/loop.py` —
  - `class TokenLedger` constructed with `(path: Path, per_cycle_cap: int, weekly_cap: int)`. On-disk shape is a JSON `list[{"at": <iso8601 str>, "tokens": <int>}]` (one entry per `record` call); `remaining_this_week` sums `tokens` over entries whose `at` is within the rolling 7-day window of `now`. `def remaining_this_cycle(self) -> int` / `def remaining_this_week(self, now: datetime) -> int` (week = rolling 7-day window over recorded usage). `def record(self, tokens: int, now: datetime) -> None` (persist atomically). `def can_spend(self, now: datetime) -> bool`: True iff both caps have headroom. — the HARD cap: when False the cycle is a no-op. (Wall-clock; the rolling 7-day window is not clock-skew-hardened — a backward clock jump could let one week exceed the cap; documented accepted risk for a single-box deployment, not a crash path. Same-dir atomic persist.)
...
- [ ] Task 6: Write the Curiosity-loop tests (off-hardware, fakes) — files: `/Users/artemis-build/artemis/tests/test_curiosity_loop.py` — typed pytest with `FakeTelemetry` (returns canned escalation/confidence/topic/stale data), `StubResearcher` (injectable passing/failing result), `FakeReachability` (configurable reachable set), `FakeModelPort` (reuse M7-a fake), a real `RecipeStore`/`StagingStore`/`TokenLedger` over `tmp_path`. Tests:
  ...
  - chunk-commit not-yet-wired: `commit_staged(item_id)` for a `chunk`-kind staged item raises `NotImplementedError` (the M3 ingest hook is not yet specced).
...
| Command | Purpose |
|---------|---------|
| `uv add tldextract` | New runtime dep — eTLD+1 helper (`registrable_domain`) for the grounding gate; verify package name + maintainer before adding |
| `uv run pip-audit` | Supply-chain gate on tldextract + its transitive deps; must exit 0 with no known vulns |
...
- [ ] Run `uv run pip-audit` → verify: exit 0, no known vulnerabilities in tldextract's dependency tree.
