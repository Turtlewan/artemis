...
- [ ] Task 2: Implement the recipe store + RAG-for-recipes index — files: `/Users/artemis-build/artemis/src/artemis/recipes/store.py` —
  ...
  - `class RecipeStore` constructed with `(embedder: EmbeddingModel, recipes_dir: Path, index: VectorStore | None = None, signer: RecipeSigner | None = None)`. Methods:
    ...
    - `async def retrieve_recipes(self, query, k=3, *, status: RecipeStatus | None = RecipeStatus.ENABLED) -> list[str]`: embed query via `await self.embedder.embed(...)` (`EmbeddingModel.embed` async — ADR-015), `index.search` (`VectorStore.search` stays sync — no `await`), return recipe names (default scoped to `ENABLED` — RAG-for-recipes, exactly like RAG-for-tools).
...
- [ ] Task 5: Write the model+store+signing tests — files: `/Users/artemis-build/artemis/tests/test_recipes_store_signing.py` — typed pytest with `FakeEmbedder` (deterministic hash-based vectors, reuse M1-a pattern; `async def embed(...)` — `EmbeddingModel.embed` is async per ADR-015; `dimension` stays a sync property) + `FakeKeyProvider` (fixed bytes), a real `RecipeStore` over `tmp_path`. The store-touching tests are `async def` (mark per the repo's pytest-asyncio convention; if none is established, add `pytest.mark.asyncio` and note it) and `await` the async store methods:
