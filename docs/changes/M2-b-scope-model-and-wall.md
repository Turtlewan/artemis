...
- Each scope gets a SEPARATE vector-index handle. Per ADR-004, owner *memory* vectors live INSIDE the per-scope SQLCipher file (sqlite-vec) — NOT LanceDB; LanceDB stays for the non-sensitive document corpus (owner-private/general only, M3). In M2 the "vector-index handle" is a typed HANDLE/locator object per scope (not an initialised engine — engines are M3/M4); M2 proves the handles are distinct per scope and that a guest scope gets its own. → impact: Caution (M2 provisions handles + the wall; engine init is later). The LanceDB doc-corpus index for owner scopes lives inside the per-scope encrypted volume mounted by the broker (ADR-007); in M2-b the handle is just a locator under `scope_dir`, the actual mounted-volume path is contracted by M2-a/M2-c.
...
- [ ] Task 5: Write the scope + wall tests — files: `/Users/artemis-build/artemis/tests/test_scope_wall.py` — typed pytest:
  ...
  - provisioning isolation: provision `owner-private` and `guest-bob` with a fake `broker_provision` recording calls + a `FakeKeyProvider` seeded post-provision; assert each `ScopedStore.db_path()` is under its OWN `scope_dir`, the two paths differ, and `vector_index_handle()` returns distinct objects per scope.
