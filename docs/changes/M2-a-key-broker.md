...
- [ ] Task 10: Implement the per-scope encrypted-volume mount/unmount — files: `/Users/artemis-build/artemis/swift/ArtemisBroker/Sources/ArtemisBroker/VolumeMount.swift` — `struct VolumeMount`: `mountPoint(scope:) -> URL` = `/opt/artemis/<slot>/<scope>/vault/`; `mount(scope:) throws` attaches the scope's encrypted APFS volume/sparsebundle (key SE-unwrapped, same `.userPresence` policy) at that mount point; `unmount(scope:) throws`; `isMounted(scope:) -> Bool`. Wire into the broker: `getDEK` mounts on a successful proof BEFORE returning the DEK; `lock`/idle/`lockAll`/process-exit unmount. The concrete attach mechanism (hdiutil sparsebundle vs `diskutil apfs` encrypted volume) is the ADR-007 mount-lifecycle build-time spike → the real attach is GATED on-hardware (folded into the gated Task 9). Off-hardware the mount is a verifiable no-op shim recording mount/unmount calls. — done when: `swift build` passes; an off-hardware test asserts `getDEK` triggers `mount(scope)` before returning the DEK and `lock` triggers `unmount(scope)`; real attach gated on-hardware (Task 9).
