...
### SC1a — DeepSeek distill judge shell environment
The standalone `tools/distill` judge reads DeepSeek settings directly from the process environment.
Set `$env:DEEPSEEK_API_KEY = "..."` in PowerShell before a real run. Optional overrides:
`DEEPSEEK_BASE_URL` defaults to `https://api.deepseek.com/v1`, and `DEEPSEEK_JUDGE_MODEL`
defaults to `deepseek-chat`. The pipeline does not load `.env` files.
