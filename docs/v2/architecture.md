...
1. **Memory engine — RESOLVED + CONFIRMED by spike (2026-06-30): Cognee-first behind `MemoryPort`.** Cognee is embedded (Kuzu+LanceDB+SQLite), pure-Python, no server, LiteLLM-aligned → runs in-process here; Graphiti needs a graph-DB *server* (FalkorDB/Neo4j via Docker/WSL2) = added Windows friction. **The Cognee-vs-Graphiti spike ran live on LoCoMo** (both engines, subscription `gpt-5.5` via a `codex exec` proxy, local Ollama embeddings) → **Cognee confirmed**: in-process/zero-infra, ~4× faster ingest, ~4× fewer LLM calls (= less subscription quota), robust extraction even on a local 4B; Graphiti needs Docker+FalkorDB, is ~4× heavier, fragile with weak models, and its bi-temporal edge was *not* shown material at the tested scale (deeper temporal test deferred, not funded). Full verdict: `docs/findings/cognee-vs-graphiti-spike-2026-06-30.md`. Build Slice 2 on Cognee + port a decay/temporal policy on top; revisit Graphiti only if "what-was-true-when" proves weak in real use.
