...
- **D1 mechanism (pinned 2026-06-24 — fixes the CAL-b re-dispatch loop):** `ToolSpec` carries an optional **`execute_callable_ref: Callable[..., Awaitable[BaseModel]] | None = None`**. At `register()`, for any `WRITE`/`HIGH_STAKES` tool the registry maps `{fq}_execute` → **`tool.execute_callable_ref` if set, else `tool.callable_ref`** (back-compatible fallback — a module whose front-door is already a raw write supplies nothing). A **runtime-gated** module (front-door = classify+stage-or-execute, e.g. CAL-b) MUST set `execute_callable_ref` to its *raw, classifier-free* method, so `approve()` dispatches the raw write and never re-enters `classify()`. The raw method is reachable **only** through the `{fq}_execute` staging path — it is never a `ToolSpec.name` in `manifest.tools`, so it stays out of `retrieve_tools()`. Without this, `{fq}_execute` aliases the front-door and `approve()` re-classifies an attendee event → re-stage loop (2026-06-10 B1).
...
**Δ:** GATE-a add `list_pending()` + the `_execute` mapping + conditional-update. GATE-b fix route param ordering (`request: Request` before defaulted `Depends` params — B3; same fix in CLIENT-b). CAL-b/c supply `execute_callable_ref` (Seam 2 D1 mechanism) for each gated tool so `approve` dispatches the raw twin and stops re-running the gated entrypoint.
