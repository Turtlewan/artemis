...
| [034](../adr/ADR-034-unified-voice-text-ask.md) | **Unified voice + text Ask**: the Ask overlay accepts spoken **and** typed input and answers in streamed on-screen text **and** spoken aloud. One `Brain.respond_stream` generation **teed** into display (SSE, unchanged) + speak (brain-side speakable → M5-d splitter → TTS → sidecar). Speakable = deterministic, **shape-aware** (short→speak verbatim; list/long→speak a pointer, never read aloud); two-field model output rejected. Overlay spoken input = session-scoped (not voice-ID auth). Co-located brain-mic now; remote client-mic deferred |
