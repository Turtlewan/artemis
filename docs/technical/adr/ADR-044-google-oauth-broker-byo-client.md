...
- **R2 — The brain opens the consent browser (refines decision 2). [Reversed at build, 2026-07-03.]** Original R2 planned for the *client* to open the consent URL (no-op opener on the brain). Reversed during the oauth-4 build: the Tauri client has no opener/shell plugin, and adding one (Cargo + npm + capability + permission) is scope the local-desktop MVP doesn't need — the brain runs on the same machine and opens the consent URL itself via its default `webbrowser.open` opener (`create_app`, `bc632ab`). Identical UX; the client triggers connect and shows status but opens no URL. The brain-side loopback listener still catches the redirect. Revisit if the brain ever runs headless/remote from the client.
