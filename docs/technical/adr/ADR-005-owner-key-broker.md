...
> **Cross-ref 2026-06-22 — hardware root + client assertion re-expressed for Tauri → ADR-025.** **Still in force, unchanged:** the broker concept (smallest trusted base touches the host key; brain gets only a transient DEK), the **host-side DEK wrap** (DEK wrapped to a host hardware key, ciphertext on disk, unwrapped only on a verified biometric assertion), session-only DEK in mlock'd memory + idle re-lock, the **counter-checked signed-assertion** contract, and the **recovery passphrase** (§Refinement 2026-06-22). **Changed by ADR-025:** the biometric assertion no longer must originate on the **iPhone** (that was a workaround for the headless, Touch-ID-less Mini) — it comes from **any paired Tauri device with a biometric**, and the hardware primitive generalises **Secure Enclave → TPM/Windows Hello on Windows-dev hosts/clients** (CNG/NCrypt; macOS keeps SE). The R2 finding reinforces this ADR's never-PRF stance: the DEK is sealed natively, never wrapped by a passkey-PRF output. Read ADR-025 for the re-rooted details.
