...
3. **Sandbox = a no-network Windows AppContainer + Job Object** (network-deny research `docs/research/2026-06-26-windows-network-deny/README.md`). **Superseding the earlier "restricted-token + Job Object, mirror Codex" note:** the Codex `apex-coder` profile runs network-ON with the dev user's own SID — it is NOT a network sandbox. An AppContainer launched without the `internetClient*` capabilities gets a unique package SID + an automatic kernel outbound-block (loopback too), with **NO admin** (the Chromium/Edge renderer-sandbox technique). It replaces the restricted token (Job Object composes on top); the workspace is ACL-granted to the package SID. This makes command-exec network-deny **mechanically enforced**, so IN_SANDBOX command auto-run is enabled on the dev box (no gate-everything fallback) — resolving the dev-box-command-autonomy decision. Mac parity = Docker workspace `--network none`. (ctypes/userenv.dll impl; precondition: MPSSVC running, else fail-closed.)
