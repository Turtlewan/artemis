...
- **Recipe use = plan-fresh + compose verified fragments.** The planner always builds a fresh plan but reuses verified recipe FRAGMENTS where a plan-step matches. IMPLICATION: M7 recipes must be fragment-composable (addressable at sub-step level), not whole-task-only — reconcile with the M7-a1 recipe format (see follow-up a).
...
**Follow-ups:** (a) M7-a1 recipe format must support fragment-level composability; (b) confirm whether intra-GPT model tiering works on the Codex ChatGPT-subscription path or needs the metered OpenAI API [under research 2026-06-23].
