...
- **Agency (A) — LOCKED to ONE narrow action: schedule a task, internal store only.** Artemis may place a task into a time slot in its **own owned productivity store** (uses the built focus-window slot-pick). **No Google Calendar write** (the `calendar.schedule_task` external leg stays OFF). Because internal task-scheduling is **internal-reversible**, by the owner autonomy boundary it needs **no approval GATE**.
  - **Trigger model — REFINED to suggest/accept ONLY (Option B1, after security review 2026-06-28).** Artemis may `suggestion.create` a proposed slot (digest or on request); scheduling happens ONLY when the owner `suggestion.accept`s. The schedule-write is NOT a free reactive tool, so "never auto" is enforced **structurally** (no model→schedule code path), not by convention. Free-form "just schedule X for tomorrow" is **dropped for now** — it would need the deferred WRITE-confirm GATE (Option B2, rejected), so it arrives later with the GATE lever.
  - **Security folds (review 2026-06-28):** registering the task module must use `include_write_surface=False` (BLOCK-1: otherwise `tasks.cancel`/`complete`/`update` go live ungated, and cancel is NOT reversible); the accept-handler asserts no calendar coupling (FLAG-3); externally-sourced suggestion fields are truncated/stripped (FLAG-4); locked vault → task tools 423/no-op. Full spec: `docs/drafts/agency-internal-task-schedule.md`.
