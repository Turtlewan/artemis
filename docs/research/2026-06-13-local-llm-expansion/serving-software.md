...
- **MTPLX** (native MTP runtime, *added 2026-06-16 from a field video*): MLX server that bakes **multi-token prediction (MTP)** — a speculative-decoding scheme — into the model, fusing drafter+verifier into one artifact (no second draft model to load). Ships a per-Mac auto-tuner. Same OpenAI-compatible API as the others → **drop-in swap behind the M0-c runtime seam** (config-only per `artemis-integration-constraints.md`). Appeal vs a smaller model: the verifier only emits tokens the full model would have produced, so the speed-up is **quality-neutral** — fits the latency-sensitive M5 voice loop + M6 heartbeat. Caveats: (1) **brand new** — maturity/stability/supply-chain risk on a core dependency; (2) the field claim (~23% faster tokgen, one confounded run — reviewer ran OBS + two engines at once; historically "mixed results" with MTP) needs **on-device A/B** vs `mlx-openai-server` and `vllm-mlx`, can't be assessed without the Mini; (3) **model lock-in** to their MTP-built variants (fights Bank decision A model-flexibility); (4) the real feature is *speculative decoding* — confirm whether `mlx-lm`/`vllm-mlx` expose it natively on the already-trusted runtime before adopting a new server. **Status: benchmark candidate only, not a swap decision.**
